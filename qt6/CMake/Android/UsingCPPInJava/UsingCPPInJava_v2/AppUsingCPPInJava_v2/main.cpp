...
Q_DECLARE_JNI_CLASS(AJavaClassCallback,"systems/arboreus/AJavaPackage/AJavaClassCallback")

static jint mTestMethod(JNIEnv* inEnvironment, jobject inObject, jint inNumber) {

	Q_UNUSED(inEnvironment);
	Q_UNUSED(inObject);

	_A_DEBUG << "JAVA native callback:" << inNumber;

	return inNumber * 3;
}

Q_DECLARE_JNI_NATIVE_METHOD(mTestMethod)
...
Q_DECL_EXPORT jint JNICALL JNI_OnLoad(JavaVM* inJavaVM, void* inReserved) {
	...
	oJNIEnvironment.registerNativeMethods<QtJniTypes::AJavaClassCallback>({
		Q_JNI_NATIVE_METHOD(mTestMethod)
	});
