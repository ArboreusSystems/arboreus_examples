...
void ABackend::mInit(
	...
	pNotifications = new ANotifications(pQmlApplicationEngine);

	QObject::connect(
		pApplication,&AApplication::sgInitiated,
		this,[this](){
			this->mInitNotifications();
		}
	);
	QObject::connect(
		pNotifications,&ANotifications::sgInitiated,
		this,[this](){
...
void ABackend::mInitNotifications() {

	pNotifications->mInit();
	pRootContext->setContextProperty("ANotifications",pNotifications);
	pRootContext->setContextProperty("ANotificationsTemplates",pNotifications->pTemplates);
}
