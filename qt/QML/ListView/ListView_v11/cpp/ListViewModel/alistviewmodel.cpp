...
QVariantList AListViewModel::mOrderIndex(void) {

	QVariantList oOutput = {};
	QList<QString> oOrder = {};

	for (int i = 0; i < pRowCount; i++) {

		QString iFirstLetter = qvariant_cast<QVariantList>(pCache[i])[pIndex].toString().at(0);

		if (!oOrder.contains(iFirstLetter)) {

			oOrder.append(iFirstLetter);

			ADBOrderProperty oOrderProperty;
			oOrderProperty.Text = iFirstLetter;
			oOrderProperty.Index = i;
			oOutput.append(oOrderProperty.mToVariantMap());
		}

		_A_DEBUG << "First letter:" << iFirstLetter;
	}
