...
QVariantList AStorage::mGetTableInfo(void) {

	QEventLoop oEventLoop;

	AThreadObjectControllerTemplate oController;
	ADBRequestTableInfo oRequest(this->mService()->mGetDB());

	QObject::connect(
		&oController,&AThreadObjectControllerTemplate::sgRun,
		&oRequest,&ADBRequestTableInfo::slRun
	);
	QObject::connect(
		&oRequest,&ADBRequestTableInfo::sgFinished,
		&oEventLoop,&QEventLoop::quit
	);
	oRequest.moveToThread(this);
...
QVariantList AStorage::mGetTableFieldNames(void) {

	QEventLoop oEventLoop;

	AThreadObjectControllerTemplate oController;
	ADBRequestTableFieldNames oRequest(this->mService()->mGetDB());

	QObject::connect(
		&oController,&AThreadObjectControllerTemplate::sgRun,
		&oRequest,&ADBRequestTableFieldNames::slRun
	);
	QObject::connect(
		&oRequest,&ADBRequestTableFieldNames::sgFinished,
		&oEventLoop,&QEventLoop::quit
	);
	oRequest.moveToThread(this);
