...
ADBSqlCipherReply ADBSqlCipher::mGetTableFieldNames(QString inTableName) {

	ADBSqlCipherReply oReply = this->mGetTableInfo(inTableName);
	QVariantList oOutput;

	if (oReply.Status) {
		for (int i = 0; i < oReply.Output.length(); i++) {
			QString oRole = qvariant_cast<QString>(
				qvariant_cast<QVariantList>(oReply.Output[i])[1]
			);
			oOutput.append(oRole);
		}
	}
