	...
	Rectangle {
		...
		AContentWrapper {
			...
			Component.onCompleted: {
				...
				var oSafeAreaHeightOffset = oSafeAreaTop.height + oSafeAreaBottom.height;
				oContentWrapper.height = oWindowWrapper.height - oSafeAreaHeightOffset;

				var oContentData;
				var oDeviceType = AGlobal.mDeviceType();
				if (oDeviceType === GLOBAL.DEVICE_TYPE_TABLET) {
					oContentData = Qt.createQmlObject(
						COMPONENTS.mAContentTablet(),oContentWrapper,"oContent"
					);
				} else if (oDeviceType === GLOBAL.DEVICE_TYPE_PHONE) {
					oContentData = Qt.createQmlObject(
						COMPONENTS.mAContentPhone(),oContentWrapper,"oContent"
					);
