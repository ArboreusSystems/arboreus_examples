...
MatrixXd AETL::mNormalize(MatrixXd inMatrix) {

	auto oMean = mMean(inMatrix);
	Eigen::MatrixXd oScaledMatrix = inMatrix.rowwise() - oMean;
	auto oStd = mStd(oScaledMatrix);
	Eigen::MatrixXd oNormalizedMatrix = oScaledMatrix.array().rowwise() / oStd;
