...
start_link() ->
	...
	io:format("a_yapp_v2_sup starting link"),
...
init([]) ->
	...
	io:format("a_yapp_v2_sup initiating~n"),
	...
	case supervisor:check_childspecs(ChildSpecifications) of
		ok ->
			...
			io:format("a_yapp_v2_sup initiated~n"),
			{ok, {RestartStrategy,ChildSpecifications}};
		{error, Reason} ->
			io:format("a_yapp_v2_sup ERROR: Wrong childspecifications~n"),
			{error,Reason}
	end.
