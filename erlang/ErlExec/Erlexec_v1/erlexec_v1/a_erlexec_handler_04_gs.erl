...
%%%-------------------------------------------------------------------
-module(a_erlexec_handler_04_gs).
-author("Alexandr Kirilov, https://alexandr.kirilov.me").
...
test(COMMAND) ->
	...
	erlang:display({
		calling_start,
		gen_server:call(HANDLER_PID,{start_process,COMMAND})
	}),
...
handle_call({start_process,COMMAND},_FROM,STATE) ->

	erlang:display({handle_call,start_process}),
	{reply,run_command(COMMAND),STATE};
...
handle_info({'DOWN',OS_PID,TYPE,ERLEXEC_PID,STATUS},STATE) ->

	erlang:display({handle_info_down,{'DOWN',OS_PID,TYPE,ERLEXEC_PID,STATUS}}),
	{noreply, STATE};
