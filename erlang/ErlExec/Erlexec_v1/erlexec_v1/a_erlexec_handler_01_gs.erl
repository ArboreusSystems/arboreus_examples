...
%%%
%%% @end
%%% Created : 06. Feb 2025 11:03
%%%-------------------------------------------------------------------
-module(a_erlexec_handler_01_gs).
-author("Alexandr Kirilov, https://alexandr.kirilov.me").
...
%% State
-record(a_erlexec_handler_01_gs_state, {}).
...
test() ->
	...
	{ok,HANDLER_PID} = start_link(#a_erlexec_handler_01_gs_state{}),
	erlang:display({handler,HANDLER_PID}),

	timer:sleep(2000),

	START_PROCESS_01 = exec:run(
		"echo ok",[
			{stderr,HANDLER_PID},
			{stdout,HANDLER_PID},
			monitor
	...
	erlang:display({start_process_01,START_PROCESS_01}),

	timer:sleep(2000),

	START_PROCESS_02 = exec:run(
		"echo ok",[
			{stdout, fun(S,OsPid,D) -> io:format("Got ~w from ~w: ~p\n", [S,OsPid,D]) end}
		]
	),
	erlang:display({start_process_02,START_PROCESS_02}),

	timer:sleep(2000),

	START_PROCESS_03 = exec:run(
		"echo ok",[
			{stdout, fun(S,OsPid,D) -> gen_server:call(HANDLER_PID,[S,OsPid,D]) end}
		]
	),
	erlang:display({start_process_03,START_PROCESS_03}),

	receive
		MESSAGE ->
			erlang:display({received,MESSAGE}),
			gen_server:call(HANDLER_PID,{received,MESSAGE})
	end,
...
-spec start_link(STATE) -> {ok, PID} | ignore | {error, REASON}
	when
		STATE :: #a_erlexec_handler_01_gs_state{},
		PID :: pid(),
		REASON :: term().
...
-spec handle_call(REQUEST, FROM, STATE) ->
	...
	when
		...
		REPLY :: term(),
		STATE :: #a_erlexec_handler_01_gs_state{},
		NEW_STATE :: #a_erlexec_handler_01_gs_state{},
		TIMEOUT :: timeout() | hibernate,
		REASON :: term().

handle_call(REQUEST, FROM, STATE) ->

	erlang:display({handle_call,REQUEST,FROM}),
	{reply, ok, STATE}.
...
handle_info({'EXIT', _FROM, REASON}, STATE) -> {stop, REASON, STATE};
